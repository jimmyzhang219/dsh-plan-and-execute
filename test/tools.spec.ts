...
const run = (
  tool: { execute: (args: unknown, exec: never) => Promise<unknown> },
  args: unknown,
...
describe('report_step 工具', () => {
  ...
  it('outcome 非法 → 抛错', async () => {
    ...
    await expect(run(tool, { outcome: 'oops', summary: 'x' }))
      .rejects.toThrow("outcome 必须是 'done' 或 'blocked'")
  })
})
...
describe('reportStepForCurrent', () => {
  it('非执行期（批准后无当前步）→ 抛错', async () => {
    ...
    expect(() => orchestrator.reportStepForCurrent('done', '太早'))
      .toThrow('report_step 仅在执行阶段的当前步骤内可用')
  })
})
